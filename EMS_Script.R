...
EMS_2024_Before = EMS_2024 %>% filter(Date <= '2024-04-06') %>% mutate(Period = "Before 04/07/24")
EMS_2024_After = EMS_2024 %>% filter(Date > '2024-04-06')%>% mutate(Period = "After 04/07/24")
EMS_2025_Before = EMS_2025 %>% filter(Date <= '2025-04-06')%>% mutate(Period = "Before 04/07/25")
EMS_2025_After = EMS_2025 %>% filter(Date > '2025-04-06')%>% mutate(Period = "After 04/07/25")
...
Daily_Call_Stats = Daily_calls_vols %>%
  group_by(Period) %>%
  summarise(
    mean_value = mean(N),
    sd_value = sd(N)
...
Daily_Call_Volumes = ggplot(Daily_calls_vols, aes(x = Period, y = N, fill = Period)) +
  ...
  scale_fill_manual(labels = c("Before 04/07/24", "After 04/07/24", "Before 04/07/25", "After 04/07/25"), 
                    values = c("firebrick", "red", "dodgerblue", "cyan")) +
  geom_text(data = Daily_Call_Stats, aes(x = Period, y = max(Daily_calls_vols$N) + 2,
...
Daily_Calls_2025 = Daily_Calls_2025 %>%
  mutate(Period = factor(Period, levels = c("Before 04/07/25", "After 04/07/25")))
...
LCats_ByPeriod_Chart = ggplot(Largest_Cats_ByPeriod, aes(x = Description, y = Count, fill=Period)) +
  ...
  labs(title = "Number Of 10 Most Frequent Call Descriptions",
       x = "Description",
       y = "Count") +
...
MR_Df %>%
  group_by(Period) %>%
  summarise(
    mean_value = mean(Daily_Count),
    sd_value = sd(Daily_Count)
  )
...
#smaller descriptions that had different post-CAHOOTS patterns
all_dates <- tibble(
  Date = c(
    seq(as.Date("2024-02-24"), as.Date("2024-05-19"), by = "day"),
    seq(as.Date("2025-02-24"), as.Date("2025-05-19"), by = "day")
  )
) %>%
  mutate(Period = case_when(
    Date < as.Date("2024-04-07") ~ "Before 04/07/24",
    Date >= as.Date("2024-04-07") & Date <= as.Date("2024-05-19") ~ "After 04/07/24",
    Date >= as.Date("2025-02-24") & Date < as.Date("2025-04-07") ~ "Before 04/07/25",
    Date >= as.Date("2025-04-07") ~ "After 04/07/25"
  ))
...
PA_Df <- bind_rows(
  ...
  filter(Description == "PUBLIC ASSIST- FIRE/EMS") %>%
  mutate(Date = as.Date(Date)) %>%
  group_by(Date) %>%
  summarise(Daily_Count = n(), .groups = "drop")

PA_Df <- all_dates %>%
  left_join(PA_Df, by = "Date") %>%
  mutate(Daily_Count = replace_na(Daily_Count, 0)) %>%
  mutate(Period = factor(Period, levels = c("Before 04/07/24", "After 04/07/24", "Before 04/07/25", "After 04/07/25")))

PA_Dist = ggplot(PA_Df, aes(x = Period, y = Daily_Count, fill=Period)) +
  geom_boxplot() +
  labs(title = "PUBLIC ASSIST Call Distributions",
       x = "Period",
       y = "PUBLIC ASSIST Calls Per Day") +
  scale_fill_manual(labels = c("Before 04/07/24", "After 04/07/24", "Before 04/07/25", "After 04/07/25"), 
                    values = c("firebrick", "red", "dodgerblue", "cyan")) +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, vjust = 1)) +
...
PA_2025 <- PA_Df %>%
  filter(Date >= as.Date("2025-02-24"))

wilcox.test(Daily_Count ~ Period, data = PA_2025)

LA_Before24 = EMS_2024_Before %>%
  filter(Description == "LIFT ASSIST") %>%
  mutate(Period = "Before 04/07/24") %>%
  select(Description, Date, Period)

LA_Before25 = EMS_2025_Before %>%
  filter(Description == "LIFT ASSIST") %>%
  mutate(Period = "Before 04/07/25") %>%
  select(Description, Date, Period)

LA_After24 = EMS_2024_After %>%
  filter(Description == "LIFT ASSIST") %>%
  mutate(Period = "After 04/07/24") %>%
  select(Description, Date, Period)

LA_After25 = EMS_2025_After %>%
  filter(Description == "LIFT ASSIST") %>%
  mutate(Period = "After 04/07/25") %>%
  select(Description, Date, Period)

LA_Df = rbind(LA_Before24, LA_Before25, LA_After24, LA_After25) %>%  
  group_by(Date, Period) %>%
  summarise(Daily_Count = n()) %>%
  mutate(Period = factor(Period, levels = c("Before 04/07/24", "After 04/07/24", "Before 04/07/25", "After 04/07/25")))

LA_Dist = ggplot(LA_Df, aes(x = Period, y = Daily_Count, fill=Period)) +
  geom_boxplot() +
  labs(title = "LIFT ASSIST Call Distributions",
       x = "Period",
       y = "LIFT ASSIST Calls Per Day") +
  scale_fill_manual(labels = c("Before 04/07/24", "After 04/07/24", "Before 04/07/25", "After 04/07/25"), 
                    values = c("firebrick", "red", "dodgerblue", "cyan")) +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, vjust = 1))
...
TBF_Before24 = EMS_2024_Before %>%
  filter(Description == "TRASH BIN FIRE") %>%
  mutate(Period = "Before 04/07/24") %>%
  select(Description, Date, Period)

TBF_Before25 = EMS_2025_Before %>%
  filter(Description == "TRASH BIN FIRE") %>%
  mutate(Period = "Before 04/07/25") %>%
  select(Description, Date, Period)

TBF_After24 = EMS_2024_After %>%
  filter(Description == "TRASH BIN FIRE") %>%
  mutate(Period = "After 04/07/24") %>%
  select(Description, Date, Period)

TBF_After25 = EMS_2025_After %>%
  filter(Description == "TRASH BIN FIRE") %>%
  mutate(Period = "After 04/07/25") %>%
  select(Description, Date, Period)

TBF_Df = rbind(TBF_Before24, TBF_Before25, TBF_After24, TBF_After25) %>%  
  group_by(Date, Period) %>%
  summarise(Daily_Count = n()) %>%
  mutate(Period = factor(Period, levels = c("Before 04/07/24", "After 04/07/24", "Before 04/07/25", "After 04/07/25")))

TBF_Dist = ggplot(TBF_Df, aes(x = Period, y = Daily_Count, fill=Period)) +
  geom_boxplot() +
  labs(title = "TRASH BIN FIRE Call Distributions",
       x = "Period",
       y = "TRASH BIN FIRE Calls Per Day") +
  scale_fill_manual(labels = c("Before 04/07/24", "After 04/07/24", "Before 04/07/25", "After 04/07/25"), 
                    values = c("firebrick", "red", "dodgerblue", "cyan")) +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5, vjust = 1))
